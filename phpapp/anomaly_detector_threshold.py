"""
anomaly_detector.py — Pipeline flexibil pentru detectie anomalii
=================================================================
Suporta 4 combinatii train/test:

  1. Antreneaza pe CSV, testeaza pe CSV (split intern)
     python anomaly_detector_treshold.py --train-csv payload_train.csv --test-csv payload_test.csv

  2. Antreneaza pe CSV, testeaza pe LOG
     python anomaly_detector_treshold.py --train-csv payload_train.csv --test-log dataset_test_v2.log

  3. Antreneaza pe LOG, testeaza pe LOG (split intern)
     python anomaly_detector_treshold.py --train-log dataset_train_v2.log --test-log dataset_test_v2.log

  4. Antreneaza pe LOG, testeaza pe CSV
     python anomaly_detector_treshold.py --train-log dataset_train_v2.log --test-csv payload_test_v2.csv
...
def load_mixed(csv_path, log_path):
    """Combina CSV + LOG intr-un singur DataFrame de antrenare (80% din fiecare)."""
    print(f"[+] Incarcare TRAIN MIXED:")
    ...
    df_csv_train, _ = train_test_split(df_csv, test_size=0.2,
                                        random_state=42, stratify=df_csv["label"])
    df_log_train, _ = train_test_split(df_log, test_size=0.2,
                                        random_state=42, stratify=df_log["label"])

    df_mixed = pd.concat([df_csv_train, df_log_train], ignore_index=True)
    df_mixed = df_mixed.sample(frac=1, random_state=42).reset_index(drop=True)

    print(f"    CSV train: {len(df_csv_train)} | LOG train: {len(df_log_train)}")
    print(f"    Total mixed: {len(df_mixed)} randuri")
    print(f"    Labels:  {df_mixed['label'].value_counts().to_dict()}")
    print(f"    Attacks: {df_mixed['attack_type'].value_counts().to_dict()}")
    return df_mixed
...
def find_optimal_threshold(y_true, scores, strategy="f1"):
    """
    Gaseste thresholdul optim pe baza scorurilor Isolation Forest.
    Strategii:
      'f1'       - maximizeaza F1 binar anomaly (recomandat general)
      'f1_macro' - maximizeaza F1 macro (ambele clase)
      'recall90' - recall anomaly >= 0.90 cu cel mai mare threshold
    """
    ...
    if strategy == "f1":
        ...
        print(f"\n[+] Threshold optim (f1):      {best_thr:.4f}  "
              f"| precision={precision[best_idx]:.3f}  recall={recall[best_idx]:.3f}  f1={f1_scores[best_idx]:.3f}")

    elif strategy == "f1_macro":
        best_thr, best_score = float(thresholds[0]), 0.0
        for thr in thresholds:
            pred  = (scores >= thr).astype(int)
            score = f1s(y_true, pred, average="macro", zero_division=0)
            if score > best_score:
                best_score, best_thr = score, float(thr)
        print(f"\n[+] Threshold optim (f1_macro): {best_thr:.4f}  | f1_macro={best_score:.3f}")

    elif strategy == "recall90":
        candidates = [(float(thr), float(rec))
                      for thr, rec in zip(thresholds, recall[:-1]) if rec >= 0.90]
        best_thr = max(candidates, key=lambda x: x[0])[0] if candidates else float(thresholds[int(np.argmax(recall[:-1]))])
        print(f"\n[+] Threshold optim (recall90): {best_thr:.4f}")
    else:
        raise ValueError(f"Strategie necunoscuta: {strategy}. Alege: f1, f1_macro, recall90")

    return best_thr, precision, recall, thresholds


def evaluate(iso, rf, le, df_test, output_prefix, no_plots=False,
             threshold_strategy="f1"):
    """Evalueaza modelele pe df_test cu threshold default si optim."""
    ...
    iso_pred_default = (iso.predict(X) == -1).astype(int)
    default_thr      = float(np.percentile(iso_score, (1 - iso.contamination) * 100))

    if len(np.unique(y_binary)) > 1:
        opt_thr, prec_curve, rec_curve, thr_curve = find_optimal_threshold(
            y_binary, iso_score, strategy=threshold_strategy)
    else:
        opt_thr = default_thr
        prec_curve = rec_curve = thr_curve = None
        print("[!] Doar o singura clasa in test — threshold tuning dezactivat")

    iso_pred_tuned = (iso_score >= opt_thr).astype(int)
    ...
    df_test["iso_pred_default"] = iso_pred_default
    df_test["iso_pred_tuned"]   = iso_pred_tuned
    df_test["rf_predicted"]     = le.inverse_transform(rf_pred)

    print("\n" + "="*55)
    print(f"ISOLATION FOREST — threshold DEFAULT ({default_thr:.4f})")
    print("="*55)
    print(classification_report(y_binary, iso_pred_default,
                                  target_names=["normal", "anomaly"],
                                  zero_division=0))

    print("="*55)
    print(f"ISOLATION FOREST — threshold OPTIM ({opt_thr:.4f}, strategie={threshold_strategy})")
    print("="*55)
    print(classification_report(y_binary, iso_pred_tuned,
                                  target_names=["normal", "anomaly"],
                                  zero_division=0))
    ...
    _plot(df_test, y_binary, iso_pred_default, iso_pred_tuned,
          iso_score, opt_thr, default_thr,
          prec_curve, rec_curve, thr_curve,
...
def _plot(df, yb_true, iso_pred_default, iso_pred_tuned,
          iso_score, opt_thr, default_thr,
          prec_curve, rec_curve, thr_curve,
          ym_true, rf_pred, le, prefix, no_plots):
    fig, axes = plt.subplots(2, 3, figsize=(18, 10))
    fig.suptitle(f"Results — {prefix}", fontsize=14)

    # Confusion matrix ISO default
    cm1 = confusion_matrix(yb_true, iso_pred_default)
    ConfusionMatrixDisplay(cm1, display_labels=["Normal", "Anomaly"]).plot(
        ax=axes[0][0], colorbar=False)
    axes[0][0].set_title(f"ISO — default threshold ({default_thr:.3f})")

    # Confusion matrix ISO tuned
    cm_tuned = confusion_matrix(yb_true, iso_pred_tuned)
    ConfusionMatrixDisplay(cm_tuned, display_labels=["Normal", "Anomaly"]).plot(
        ax=axes[0][1], colorbar=False)
    axes[0][1].set_title(f"ISO — optimal threshold ({opt_thr:.3f})")

    # Precision-Recall curve + thresholds
    if prec_curve is not None:
        axes[0][2].plot(rec_curve[:-1], prec_curve[:-1], color="#1565C0", lw=2)
        # Mark default threshold
        scores_arr = iso_score
        def_pred   = (scores_arr >= default_thr).astype(int)
        from sklearn.metrics import precision_score, recall_score
        dp = precision_score(yb_true, def_pred, zero_division=0)
        dr = recall_score(yb_true, def_pred, zero_division=0)
        axes[0][2].scatter([dr], [dp], color="#F44336", s=100, zorder=5, label=f"Default ({default_thr:.3f})")
        # Mark optimal threshold
        op = precision_score(yb_true, iso_pred_tuned, zero_division=0)
        or_ = recall_score(yb_true, iso_pred_tuned, zero_division=0)
        axes[0][2].scatter([or_], [op], color="#4CAF50", s=100, zorder=5, label=f"Optimal ({opt_thr:.3f})")
        ...
        axes[0][2].set_title("Precision-Recall — N/A")

    # Score distribution cu threshold lines
    ...
    axes[1][0].axvline(default_thr, color="#F44336", lw=2, linestyle="--",
                       label=f"Default {default_thr:.3f}")
    axes[1][0].axvline(opt_thr,     color="#4CAF50", lw=2, linestyle="-",
                       label=f"Optimal {opt_thr:.3f}")
    axes[1][0].set_xlabel("Anomaly score")
    axes[1][0].set_ylabel("Count")
    axes[1][0].set_title("Score distribution + thresholds")
    axes[1][0].legend(fontsize=7)

    # Detection rate comparison: default vs tuned
    det_def   = df.groupby("attack_type")["iso_pred_default"].mean().reindex(present).dropna()
    det_tuned = df.groupby("attack_type")["iso_pred_tuned"].mean().reindex(present).dropna()
    xi = np.arange(len(present))
    w  = 0.35
    axes[1][1].bar(xi - w/2, det_def.values   * 100, width=w, label="Default",
                   color="#F44336", alpha=0.8)
    axes[1][1].bar(xi + w/2, det_tuned.values * 100, width=w, label="Optimal",
                   color="#4CAF50", alpha=0.8)
    axes[1][1].set_xticks(xi)
    axes[1][1].set_xticklabels(present, rotation=15, fontsize=8)
    axes[1][1].set_ylabel("% detectat ca anomalie")
    axes[1][1].set_title("Detectie: Default vs Optimal threshold")
    ...
    ConfusionMatrixDisplay(cm2, display_labels=le.classes_).plot(
        ax=axes[1][2], colorbar=False)
    axes[1][2].set_title("Random Forest — multiclass")
...
def main():
    ...
    parser.add_argument("--train-mixed-csv", help="Antreneaza pe CSV+LOG combinat (partea CSV)")
    parser.add_argument("--train-mixed-log", help="Antreneaza pe CSV+LOG combinat (partea LOG)")
    parser.add_argument("--no-plots",  action="store_true", help="Nu afisa grafice")
    parser.add_argument("--threshold", default="f1",
                        choices=["f1", "f1_macro", "recall90"],
                        help="Strategie threshold tuning: f1 | f1_macro | recall90 (default: f1)")
    args = parser.parse_args()

    # Validare argumente
    is_mixed  = bool(args.train_mixed_csv or args.train_mixed_log)
    is_single = bool(args.train_csv or args.train_log)

    if is_mixed and is_single:
        parser.error("Foloseste fie --train-csv/--train-log, fie --train-mixed-csv/--train-mixed-log, nu ambele.")
    if is_mixed and not (args.train_mixed_csv and args.train_mixed_log):
        parser.error("--train-mixed necesita ambii: --train-mixed-csv SI --train-mixed-log")
    if not is_mixed and not is_single:
        parser.error("Trebuie specificat o sursa de antrenare.")
    ...
    # Detectie mode
    train_src   = "MIXED (CSV + LOG)" if is_mixed else ("CSV" if args.train_csv else "LOG")
    test_src    = "CSV" if args.test_csv else "LOG"
    same_source = not is_mixed and (
        (args.train_csv and args.test_csv and args.train_csv == args.test_csv) or
        (args.train_log and args.test_log and args.train_log == args.test_log)
    )

    print(f"\n{'='*55}")
    print(f"  Mod: train={train_src} | test={test_src}")
    if same_source:
    ...
    # Incarcare date
    if is_mixed:
        df_train = load_mixed(args.train_mixed_csv, args.train_mixed_log)
        ...
        df_train = load_source(args.train_csv, args.train_log, "TRAIN")
        df_test  = load_source(args.test_csv,  args.test_log,  "TEST")
