"""
 Are 5 combinatii train/test:

  1. Antreneaza pe CSV, testeaza pe CSV (split intern)
     python anomaly_detector_combined.py --train-csv payload_train.csv --test-csv payload_test.csv

  2. Antreneaza pe CSV, testeaza pe LOG
     python anomaly_detector_combined.py --train-csv payload_train.csv --test-log dataset_test_v2.log

  3. Antreneaza pe LOG, testeaza pe LOG (split intern)
     python anomaly_detector_combined.py --train-log dataset_train_v2.log --test-log dataset_test_v2.log

  4. Antreneaza pe LOG, testeaza pe CSV
     python anomaly_detector_combined.py --train-log dataset_train_v2.log --test-csv payload_test_v2.csv

  5. Antreneaza pe CSV + LOG combinat (mixed), testeaza pe CSV sau LOG
     python anomaly_detector_combined.py --train-mixed-csv payload_train.csv --train-mixed-log dataset_train_v2.log --test-csv payload_test.csv
     python anomaly_detector_combined.py --train-mixed-csv payload_train.csv --train-mixed-log dataset_train_v2.log --test-log dataset_test_v2.log
...
def load_mixed(csv_path, log_path):
    """Combina CSV + LOG intr-un singur DataFrame de antrenare.
    Face undersample din sursa majoritara pentru a echilibra contributiile.
    """
    ...
    # Adauga coloana sursa pentru transparenta
    df_csv["source"] = "csv"
    df_log["source"] = "log"

    # Split 80% din fiecare sursa pentru antrenare
...
def main():
    ...
    parser.add_argument("--train-csv",       help="Antreneaza pe CSV")
    parser.add_argument("--train-log",       help="Antreneaza pe LOG")
    ...
    is_single_train = bool(args.train_csv or args.train_log)

    if is_mixed and is_single_train:
    ...
    if is_mixed and not (args.train_mixed_csv and args.train_mixed_log):
        parser.error("--train-mixed necesita ambii parametri: --train-mixed-csv SI --train-mixed-log")
    if not is_mixed and not is_single_train:
    ...
    if is_mixed:
        ...
        df_full = load_source(args.train_csv, args.train_log, "TRAIN")
